...
def train():
    ...
    with mlflow.start_run(run_name=f"dpo_{run_id}") as run:
        ...
        # ── mlx-lm lora training on chosen responses ──────
        # mlx-lm has no dpo subcommand; preference-guided SFT via lora --train
        # trains only on chosen responses, which is the practical equivalent
        # for fine-tuning without a separate reference model
        cmd = [
        ...
        # caffeinate prevents macOS sleep during training — same as P1
